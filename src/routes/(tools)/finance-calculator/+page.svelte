...
{#if selectedCalculator === 'salary'}
  <div class="calculator p-6 rounded-lg shadow-lg max-w-md mx-auto mt-10 mb-10">
    ...
    <label class="block mb-2">
      HOURLY WAGE:
      <input type="number" bind:value={hourlyWage} class="border border-gray-300 rounded w-full py-2 px-3 mt-1" />
    </label>
    <label class="block mb-2">
      HOURS WORKED:
      <input type="number" bind:value={hoursWorked} class="border border-gray-300 rounded w-full py-2 px-3 mt-1" />
    </label>
    <button on:click={calculateSalary} class="bg-green-500 text-white py-2 px-4 rounded w-full font-bold">
      Calculate Net Salary
    </button>
      <p class="mt-4 text-lg font-semibold">Net Salary: {totalSalary.toFixed(2)}</p>
  </div>
{/if}
